...
def test_time_range_mapping_30d_uses_month(
...
def test_analyze_counts_duplicate_title_company_postings_separately(
    ...
    sec = next(
        item
        for item in data["data"]["certifications"]["items"]
...
async def test_fetch_jobs_expanded_does_not_collapse_same_title_company_without_job_id(
    ...
    async def fake_fetch_jobs_single(
        client, query: str, location: str | None = None, date_posted: str = "today"
    ):
        return [
            {
                "job_title": "Cybersecurity Analyst",
...
def test_analyze_echoes_personalization_fields(
    ...
    async def fake_fetch_jobs_expanded(
        job_title: str, location: str | None = None, date_posted: str = "today"
    ):
        return (
            [
                {
...
def test_save_scan_applies_retention_limits(
    ...
    conn = main._get_db()
    rows = conn.execute(
        "SELECT job_title FROM scans ORDER BY timestamp DESC"
    ).fetchall()
    conn.close()
...
def test_save_scan_retention_can_be_disabled(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
